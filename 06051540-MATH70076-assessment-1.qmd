...
cor_results <- tapply(
  ...
  gpd_samples$set_id,
  function(ii) {
    cor(gpd_samples$value[ii], expected_q[ii])   
  }
)
cor_results
...
- Values taken from f are on the $y=x$ line for smaller values but are significantly beneath the line for larger values. This, however, must be caveated by the fact that of the 98 samples in the dataset, only 4 noticably deviate from the $y=x$ line, a result which can be explained by outliers and large sigma. **CHECK THIS**  
